...
export default class HotUpdate {
  ...
  getGrayVersion(e, t, o = 0) {
    ...
    r.onreadystatechange = function () {
      if (4 == r.readyState) {
        ...
        } else if (o < 2) n.getGrayVersion(e, t, o + 1);else {
          t(false);
          SdkHelper.reportData("hot_update_failed", {
            status: r.status,
            type: "status",
            versionUrl: e,
            error: r.statusText + "," + r.status + "," + r.responseText + "," + r.readyState
          });
        }
      }
    };
    r.open("POST", e, true);
    r.setRequestHeader("Content-type", "multipart/form-data;boundary=AaB03x");
    r.send();
    r.addEventListener("abort", function () {
      if (!a) {
        a = true;
        if (o < 2) n.getGrayVersion(e, t, o + 1);else {
          t(false);
  ...
  downloadUpdate(e, t, o, n, a = 0) {
    var r = this;
    if (e && 1 == e.code) {
      var c = new jsb.Downloader();
      c.createDownloadFileTask(e.url, o, "@temp_version");
      c.setOnTaskError(function (c, s, l, u) {
        if (a < 2) r.downloadUpdate(e, t, o, n, a + 1);else {
          console.log("errorCode = ", s);
          console.log("errorCodeInternal = ", l);
          console.log("errorStr = ", u);
          console.log("error task = ", c.identifier);
          SdkHelper.reportData("hot_update_failed", {
            type: "download",
            error: s + "," + l + "," + u + "," + c.identifier,
            url: e.url,
            store: o
          });
          r._updating = false;
      ...
      c.setOnFileTaskSuccess(function (e) {
        if ("@temp_manifest" == e.identifier) {
          ...
          var i = new jsb.Manifest(n);
          r._am.loadRemoteManifest(i);
        } else {
          console.log("version download success");
          var a = (i = new jsb.Manifest(o)).getVersion();
          jsb.fileUtils.removeFile(o);
          console.log("local version", r._curVersion);
          console.log("remote version", a);
          if (r.versionCompareHandle(r._curVersion, a) < 0) {
            console.log("tempManifest", n, i.getManifestFileUrl());
            c.createDownloadFileTask(i.getManifestFileUrl(), n, "@temp_manifest");
          } else {
            r._updating = false;
            t && t(false);
          }
  ...
  checkNonReviewerVersion() {
    return "vivo" == ClientData.channel_name.toLowerCase() || "xiaomi" == ClientData.channel_name.toLowerCase() || "huawei" == ClientData.channel_name.toLowerCase() || "honor" == ClientData.channel_name.toLowerCase();
  }
}