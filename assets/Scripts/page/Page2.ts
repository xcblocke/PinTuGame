...
export default class Page2 extends PageBase {
  ...
  initUI() {
    ...
    for (var e = 0; e < 10; e++) {
      ...
      t.getChildByName("ly").getChildByName("auth").getComponent(cc.Label).string = `gkey_230??&value1==${o.author}`;
      var n = Math.floor(30 * Math.random() + 70);
      t.getChildByName("labDesc").getComponent(cc.Label).string = `gkey_231??&value1==${n}`;
      o.randomNum = n;
      EngineUtil.setNodeSprieFrame(t.getChildByName("image_side"), resBasePath + "book/" + o.img);
